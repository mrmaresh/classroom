...
def login(request):
    ...
    record_query = Record.objects.filter(timestamp__gt=time_threshold).values('student_id', 'reason').annotate(dcount=Count('student_id'))

    if len(record_query) > 0:
        for record in record_query:
            if record['dcount'] % 2 == 1 and record['reason'] == "use_restroom":
